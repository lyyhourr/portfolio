...
import { techs } from "@/data/experince";
...
export default function Experience() {
  ...
  const Card = ({ children, title }: { children: ReactNode, title: string }) => {
    return (
      <div className={` w-full border-2 border-gray-600 rounded-lg `}>
        <p className={`${lato.className} text-center text-2xl md:text-4xl text-gray-500 py-5`}>{title}</p>
        <main className="pb-3">
          {children}
        </main>
      </div>
  ...
  const ExperienceCard = ({ detail, duration, title }: { title: string, duration: string, detail: string }) => (
    <div className="flex flex-col gap-1 w-full">
      <h1 className="text-lg first-letter:uppercase">{title} {" "}
        ...
        <section className="flex flex-col xl:flex-row w-full px-3 sm:px-16 md:px-24 xl:px-16 gap-4 lg:gap-10 ">
          <Card title="Tech Used">
            <div className="grid grid-cols-2   gap-5" >
              {
                techs.map(item => (
                  <div key={item.img} className="flex ml-[30px] md:ml-[80px] xl:ml-[100px] items-center   gap-2">
                    <Image
                      key={item.img}
          ...
          <Card title="Work Experience">
            <div className="flex flex-col gap-2 pl-5">
              <div className="flex justify-center flex-col  gap-2 w-full">
                <ExperienceCard detail="Lorem ipsum dolor sit amet consectetur adipisicing elit. Reiciendis eos, porro numquam provident" duration="2 Months" title="title" />
              </div>
            </div>
          </Card>
        </section>
