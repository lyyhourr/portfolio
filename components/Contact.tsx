    ...
    <PageLayout scrollDownTo="home" scrollUpTo="about" disableScrollDown>
      <main className=" flex flex-col h-full justify-evenly">
        <section className="flex flex-col gap-16 ">
          ...
          <div className="border-2 border-black rounded-2xl grid sm:grid-cols-2 gap-10 py-2 lg:py-8 w-[90%] md:w-[70%] lg:w-[55%] xl:w-[50%] mx-auto pl-5">
            {
              contact.map(item => (
                <Link href={item.href} key={item.text} className={`flex items-center gap-4 ${item.img === "fb2" && "hover:underline"} ${item.img === "tele" && "hover:underline"}`}>
                  <Image
